###############################################################
##### @Title:  ICDAR 2023 DTT in image2: Text Manipulation Detection
##### @Time:  2023/02/18
##### @Author: Frank
##### @Describe: 
...
class build_dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        img = cv2.imread(img_path, cv2.IMREAD_UNCHANGED).astype('float32')# [h, w, c]
        mx = np.max(img)
        if mx:
        ...
        if self.train_val_flag: # train
            ...
            mask =  np.expand_dims(data['mask'], 0).repeat(2, 0) # [c, h, w]
...
def train_one_epoch(model, train_loader, optimizer, losses_dict, CFG):
    ...
    for _, (images, gt) in pbar:
        ...
        gt  = gt.to(CFG.device)  # [b, c, w, h]

        with amp.autocast(enabled=True):
            y_preds = model(images) 
         
            bce_loss = 0.5 * losses_dict["BCELoss"](y_preds, gt)
            tverskly_loss = 0.5 * losses_dict["TverskyLoss"](y_preds, gt)
...
def valid_one_epoch(model, valid_loader, CFG):
    ...
    for _, (images, gt) in pbar:
        ...
        val_dice = dice_coef(gt, y_preds).cpu().detach().numpy()
        val_jaccard = iou_coef(gt, y_preds).cpu().detach().numpy()
...
if __name__ == '__main__':
    ...
    class CFG:
        ...
        ckpt_fold = "ckpt_ddt2"
        ckpt_name = "efficientnetb1_img224224_bs4"  # for submit.
        tampered_img_paths = "../../data/train/tampered/imgs"
        tampered_mask_paths = "../../data/train/tampered/masks"
    ...
    if train_val_flag:
        ...
        for img_name in os.listdir(CFG.tampered_img_paths):
            if img_name.endswith('.jpg'): # pass other files
                img_path = os.path.join(CFG.tampered_img_paths, img_name)
                mask_img_name = img_name.replace('.jpg', '.png')  # img: .jpg, mask: .png
                mask_path = os.path.join(CFG.tampered_mask_paths, mask_img_name)
                imgs_info.append(["p_"+img_name, os.path.join(CFG.tampered_img_paths, img_name), mask_path])
